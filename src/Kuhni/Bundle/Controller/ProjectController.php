...
class ProjectController extends Controller
{
    ...
    public function allProjectsAction()
    {
        ...
        return $this->render('project/all/index.html.twig', array(
            'keys' => $resultKeys,
            'countKey' => $countKeys,
            'imageKey' => $imageKeys,
    ...
    public function projectsFilterAction(string $filter)
    {
        $slug = $filter;
        ...
        if (($offset <> 0)&&($limit <> 10)){
            ...
            $breadcrumbs = $this->get('white_october_breadcrumbs');
            $breadcrumbs->addItem("Главная", $this->get("router")->generate("homepage"));
            $breadcrumbs->addItem("Критерии", $this->get("router")->generate("all-projects"));
            $breadcrumbs->addItem("Все проекты", $this->get("router")->generate('projects-filter', ['filter' => 'kuhni-zov']));
            if ($slug != 'kuhni-zov')
                $breadcrumbs->addItem("{$this->getNameBreadParam($slug)}");

            return $this->render('project/filter/index.html.twig', array(
                'kitchens' => $result,
                'image' => $image,
                'slug' => $slug,
    ...
    public function projectAction($filter, $nameproject)
    {
        $slug = $nameproject;
        //получение каталога товаров
        $resultCatalog = $this->getCatalogresult();
        ...
        $result = $this->getDoctrine()->getManager()
            ->getRepository('KuhniBundle:Project')
            ->findOneBy(array('slug' => $nameproject));

        $id = $result->getId();
        $images = $this->getDoctrine()->getManager()
            ->getRepository('KuhniBundle:ProjectImages')
            ->findBy(array('projectId' => $id));
        ...
        $fasadesColorKuhni = $this->getDoctrine()->getManager()->getRepository('KuhniBundle:Project')
        ...
        if (!empty($fasadesColorKuhni)){
            ...
            $fasadesColor = $this->getDoctrine()->getManager()->getRepository('KuhniBundle:FasadColor')->findOneBy(array('id' => 1000));
            $imageFasadesColor[] = $this->imagePath('', 'fasad');
        }
        ...
        if (!empty($fasadesTypeKuhni)){
            ...
            $fasadesType = $this->getDoctrine()->getManager()->getRepository('KuhniBundle:FasadType')->findOneBy(array('id' => 1000));
            $imageFasadesType[] = $this->imagePath('', 'fasad');
        }
        ...
        $breadcrumbs->addItem("Все проекты", $this->get("router")->generate('projects-filter', ['filter' => 'kuhni-zov']));
        if ($slug != 'kuhni-zov')
            $breadcrumbs->addItem("{$this->getNameBreadParam($filter)}", $this->get("router")->generate('projects-filter', ['filter' => $filter]));
    ...
    private function getTitleKuhni()
    {
        return $this->getDoctrine()->getManager()
            ...
            ->findOneBy(array('name' => 'title-project'))->getSetting();
    }
